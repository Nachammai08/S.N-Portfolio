document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loadingScreen) {
        // Minimum display of 2s so the animation is visible
        setTimeout(() => {
    ...
    function updateClock() {
        ...
        // Format date: Mon, 28 Apr 2026
        const dateOpts = { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' };
        navDate.textContent = now.toLocaleDateString('en-IN', dateOpts);

        // Format time: 08:52:22 PM
        const timeOpts = { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true };
        navTime.textContent = now.toLocaleTimeString('en-IN', timeOpts).toUpperCase();
    }
    ...
    /* Golden Glitter Trail System */
    const glitterColors = [
        '#FFD700', '#FFC200', '#FFB800', '#FFAA00',
        '#FFE44D', '#FFF0A0', '#FFCC33', '#FFD966'
    ];
    let glitterParticles = [];
    let lastGlitterX = 0, lastGlitterY = 0;
    ...
    // Glitter canvas (on top of particle canvas, behind cursor)
    const glitterCanvas = document.createElement('canvas');
    glitterCanvas.id = 'glitter-canvas';
    glitterCanvas.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;z-index:9997;pointer-events:none;';
    ...
    function drawStar(ctx, cx, cy, r, rotation) {
        ...
        for (let i = 0; i < spikes * 2; i++) {
            const ang = (i * Math.PI / spikes) + (rotation * Math.PI / 180);
    ...
    function animateGlitter() {
        ...
        glitterParticles.forEach(p => {
            ...
            gCtx.shadowBlur = 10 * alpha;
            gCtx.fillStyle = p.color;

            if (p.shape === 'star') {
                drawStar(gCtx, p.x, p.y, p.size * scale, p.rotation);
                gCtx.fill();
            } else {
                gCtx.beginPath();
                gCtx.arc(p.x, p.y, p.size * scale * 0.6, 0, Math.PI * 2);
                gCtx.fill();
            }
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        // Spawn glitter when mouse moves enough
        const dx = mouseX - lastGlitterX;
        const dy = mouseY - lastGlitterY;
    ...
    const revealObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // Remove active class to allow re-animation on next scroll
                entry.target.classList.remove('active');
                
                // Optional: Reset progress bar if desired
                const bar = entry.target.querySelector('.progress-bar');
                if (bar) {
                    bar.style.width = '0';
                }
    ...
    /* 12. Stagger Entrance for Home */
    const heroElements = document.querySelectorAll('.hero-content > *');
    heroElements.forEach((el, index) => {
        el.style.animationDelay = `${index * 0.15}s`;
        el.classList.add('fade-up-entrance');
    });
    ...
    filterBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Categories that should NOT re-play the slide-in animation
            const noAnimFilter = ['college', 'poster'];

            projectWrappers.forEach(wrapper => {
                const cat = wrapper.getAttribute('data-category');
                const card = wrapper.querySelector('.reveal');
                
                if (filter === 'all' || cat === filter) {
                    wrapper.classList.remove('hidden');

                    if (card) {
                        if (noAnimFilter.includes(filter)) {
                            // Just show instantly — no slide animation
                            // Temporarily disable transition for instant state change
                            card.style.transition = 'none';
                            card.classList.add('active');
                            
                            // Re-enable transition after a brief moment for future mouse interactions/scrolling
                            setTimeout(() => {
                                card.style.transition = '';
                            }, 50);
                        } else {
                            // Re-trigger reveal animation for Websites / All
